    ...
    build: {
      ...
      typescript: {
        ...
        extendTsConfig(tsConfig) {
          ...
          tsConfig.compilerOptions.paths['@quasar/quasar-ui-qflashcard/dist/api/*'] = [
            './../../ui/dist/api/*',
          ]
        },
      },
      ...
      extendViteConf(viteConf, { isClient }) {
        ...
        viteConf.resolve.alias = [
          ...
          {
            find: /^@quasar\/quasar-ui-qflashcard\/dist\/api\/(.+)\.json$/,
            replacement: uiDir + '/dist/api/$1.json',
          },
          // Consume source styles in docs so local UI style edits HMR.
          {
            find: /^@quasar\/quasar-ui-qflashcard\/(?:dist\/)?index(?:\.rtl)?(?:\.min)?\.css$/,
